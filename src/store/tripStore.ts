...
import { Trip, TripMemberInvitation, User } from '../types';
...
interface TripState {
  ...
  // Local-only helpers (for mock mode sync)
      ...
      inviteMember: async (tripId, email, invitedBy) => {
        ...
        try {
          if (USE_FIREBASE && db) {
            ...
            set((state) => {
              // Add to local invitations list if not already invited
        ...
        try {
          if (USE_FIREBASE && db) {
            ...
            if (tripSnap.exists()) {
              ...
              await updateDoc(tripRef, {
                endRequests,
                status: 'active', // Since someone cancelled, status must remain active
              });
            }
          } else {
            set((state) => ({
      ...
      syncTrips: (userId) => {
        if (USE_FIREBASE && db) {
          ...
          const unsubscribe = onSnapshot(q, (snapshot) => {
            ...
            // Check for potential database wipe:
            // If the server list is completely empty, but we had trips in our store,
            // we flag a potential wipe state so the user doesn't lose local cache.
          ...
          // In mock mode, ensure all existing trips contain all mock users as active members
